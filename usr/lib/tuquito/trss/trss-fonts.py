...
#-Variables
userFonts = os.environ['HOME'] + '/.trss/userFonts'
...
class Fonts:
	def __init__(self):
		...
		self.window.set_title(_('Fuentes de Tuquito RSS'))
		self.treeview_domains = self.glade.get_object('treeview_domains')

		self.column1 = gtk.TreeViewColumn(_('Nuevas Fuentes'), gtk.CellRendererText(), text=0)
	...
	def addFont(self, widget):
		...
		self.glade.get_object('addFont').set_title(_('Agregar fuente'))
		self.glade.get_object('lname').set_label(_('Nombre: (ej. Foros Tuquito)'))
		self.glade.get_object('lurl').set_label(_('URL de la fuente:'))
